...
# Read data
data <- read.csv("CAPSTONE_PRIMER_7.csv", stringsAsFactors = FALSE)
...
# Verify new columns
kable(head(data[, c("time", "hour", "period", "CastID")]))
```
...
# Display a few rows
glimpse(data[, c("egg_dens", "log_egg", "adult_dens", "log_adult")])
```
...
ctd_temp_in  <- read_ctd("inshore_temp.csv",  station = "Inshore",  type = "Temp")
ctd_temp_out <- read_ctd("offshore_temp.csv", station = "Offshore", type = "Temp")

# Read and label Fluorescence CSVs
ctd_flu_in  <- read_ctd("inshore_fluoro.csv",  station = "Inshore",  type = "Fluoro")
ctd_flu_out <- read_ctd("offshore_fluoro.csv", station = "Offshore", type = "Fluoro")
